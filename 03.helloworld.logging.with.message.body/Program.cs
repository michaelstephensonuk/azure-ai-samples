...
var sourceName = "helloworld-logging-with-message-body";
...
IChatClient chatClient = new AzureOpenAIClient(
    ...
    .AsIChatClient()
    .AsBuilder()
    //Enabled sensitive data logging for message body, so that the prompt and response text are included in the trace.
    .UseOpenTelemetry(configure: o => o.EnableSensitiveData = true) 
    .Build();

Console.WriteLine("Azure OpenAI Chat - Hello World with Message Body with Application Insights");
...
using (var operation = appSource.StartActivity("run-ai-prompt-inc-message-body", ActivityKind.Server))
{
